...
void RedBlackTree::balanceColor(TreeNode *& root, TreeNode *& newNode){
	...
	while((root!=newNode)&&(newNode->getColor()==0)&&(newNode->getParent()->getColor()==0)){
		...
		if(p==grandpa->getLeftChild()){
			...
			if(uncle!=nullptr){
				...
				if(newNode==p->getLeftChild()){
					//will look like a flat line to the left of 3 nodes affected. rotating from the pivot
					//of the situation (gpa)
					//After switch the parent and grandpas color, and make newNode the parent to traverse up the list
				...
				else if(newNode==p->getRightChild()){
					/*Will be if parent is left child of gpa, and nN is right of parent.
					Looks like a zig-zag. after rotating newNode will become parent and reset parent to be correct.*/
		...
		else if(p==grandpa->getRightChild()){
			...
			if(uncle!=nullptr){
				...
				else if(newNode==p->getLeftChild()){
